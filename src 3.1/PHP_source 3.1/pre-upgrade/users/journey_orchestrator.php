    ...
    if(isset($_POST['subBut']))
{
    ...
    while($rows=mysqli_fetch_assoc($res)) 
    {
        ...
        if(!(in_array($rows['id'], $_POST['Required']))){
                $sql="UPDATE ".$_GET['customer_name']."_journey_orchestrator SET Is_Required = 'No',comments='".$comm."' WHERE id = '".$rows['id']."'";
            }
        else{
                $sql="UPDATE ".$_GET['customer_name']."_journey_orchestrator SET Is_Required = 'Yes',comments='".$comm."' WHERE id = '".$rows['id']."'";
            }
        $res1=$con->query($sql);
    }
}
	...
	<body> 
        <form method="post" action="#">
            <table id = "db_table" align="center" border="1px" style="width:600px; line-height:40px;"> 
                <tr> 
                    <th colspan="10" style = "text-align:center;"><h2> JOURNEY ORCHESTRATOR </h2></th> 
                </tr> 
                <tr>
                    <th> Name </th>
