...
?>
<?php
$sql3 = "SELECT * FROM ".$_GET['customer_name']."_ingest_jobs";
    ...
    if(isset($_POST['subBut']))
    {
        ...
        while($rows1=mysqli_fetch_assoc($res2)) 
        {
            ...
            if(!(in_array($rows1['id'], $_POST['Required']))){
                $sql="UPDATE ".$_GET['customer_name']."_ingest_jobs SET Is_Required = 'No',comments='$comm' WHERE name = '".$rows1['name']."'";
            }else{
                $sql="UPDATE ".$_GET['customer_name']."_ingest_jobs SET Is_Required = 'Yes',comments='$comm' WHERE name = '".$rows1['name']."'";
            }
            // echo $sql;
            $res3=$conn->query($sql);
        }
